...
test.beforeEach(async ({ page }) => {
  ...
  await page.goto('/guide/version');
});

test('Versio all in one page has expected h1', async ({ page }) => {
  await expect(page.getByRole('heading', { name: title, level: 1 })).toBeVisible();
});

test('Versio all in one page has expected meta title', async ({ page }) => {
  await expect(page).toHaveTitle(title);
});

test('Versio all in one page has expected meta description', async ({ page }) => {
  const metaDescription = page.locator('meta[name="description"]');
  await expect(metaDescription).toHaveAttribute('content', description);
});

test('Versio all in one page has expected meta og', async ({ page, baseURL }) => {

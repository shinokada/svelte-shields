...
const title = 'NPM Version Badge - Svelte Shields';
const description = 'NPM version badge component for Svelte Runes';
...
test('NPM version page has expected meta title', async ({ page }) => {
  await expect(page).toHaveTitle(title);
});

test('NPM version page has expected meta description', async ({ page }) => {
  const metaDescription = page.locator('meta[name="description"]');
  await expect(metaDescription).toHaveAttribute('content', description);
});

test('NPM version page has expected meta og', async ({ page, baseURL }) => {

...
const title = 'Download Badge - Svelte Shields';
const description = 'All in one download badge component for Svelte Runes';
const imgUrl = 'https://open-graph-vercel.vercel.app/api/svelte-shields';

test.beforeEach(async ({ page }) => {
  console.log(`Running ${test.info().title}`);
  await page.goto('/guide/download');
});

test('Download all in one page has expected h1', async ({ page }) => {
	await expect(page.getByRole('heading', { name: title, level: 1 })).toBeVisible();
});

test('Download all in one page has expected meta title', async ({ page }) => {
  await expect(page).toHaveTitle(title);
});

test('Download all in one page has expected meta description', async ({ page }) => {
  const metaDescription = page.locator('meta[name="description"]');
  await expect(metaDescription).toHaveAttribute('content', description);
});

test('Download all in one page has expected meta og', async ({ page, baseURL }) => {

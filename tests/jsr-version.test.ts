...
const title = 'JSR Version Badge - Svelte Shields';
const description = 'JSR version badge component for Svelte Runes';
...
test('JSR version page has expected h1', async ({ page }) => {
  await expect(page.getByRole('heading', { name: title, level: 1 })).toBeVisible();
});

test('JSR version page has expected meta title', async ({ page }) => {
  await expect(page).toHaveTitle(title);
});

test('JSR version page has expected meta description', async ({ page }) => {
  const metaDescription = page.locator('meta[name="description"]');
  await expect(metaDescription).toHaveAttribute('content', description);
});

test('JSR version page has expected meta og', async ({ page, baseURL }) => {

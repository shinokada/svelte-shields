...
test.beforeEach(async ({ page }) => {
  ...
  await page.goto('/guide/github-version');
});

test('GitHub version page has expected h1', async ({ page }) => {
  await expect(page.getByRole('heading', { name: title, level: 1 })).toBeVisible();
});

test('GitHub version page has expected meta title', async ({ page }) => {
  await expect(page).toHaveTitle(title);
});

test('GitHub version page has expected meta description', async ({ page }) => {
  const metaDescription = page.locator('meta[name="description"]');
  await expect(metaDescription).toHaveAttribute('content', description);
});

test('GitHub version page has expected meta og', async ({ page, baseURL }) => {

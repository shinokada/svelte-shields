<script lang="ts">
  ...
  const npmSrcData = $state(`https://img.shields.io/npm/${interval}/${packageName}?${styleOpt}${logoOpt}${logoColorOpt}${logoSizeOpt}${labelOpt}${labelColorOpt}${colorOpt}${cacheSecondsOpt}${link1}${link2}`)

  let githubSrcData = $state(`https://img.shields.io/github/downloads/${user}/${repo}/total?${styleOpt}${logoOpt}${logoColorOpt}${logoSizeOpt}${labelOpt}${labelColorOpt}${colorOpt}${cacheSecondsOpt}${link1}${link2}`)
...
{:else}
  {#if source === 'npm'}
    <!-- NPM -->
    <img  
    src={npmSrcData} alt="NPM downloads" 
    class={classname}
    {...attributes}
  />
  {:else}
    <!-- GitHub -->
    <img  
    src={githubSrcData} alt="GitHub downloads" 
    class={classname}
    {...attributes}
  />
  {/if}

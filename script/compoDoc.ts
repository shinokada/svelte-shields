...
function inferRestPropsType(typesContent: string, typeName: string): string {
	...
	if (extendsMatch) {
		...
		const htmlAttrType = baseTypes.find(
			(t) =>
				t.includes('HTMLAttributes') || t.includes('SVGAttributes') || t === 'HTMLImgAttributes'
		);
		if (htmlAttrType) {
			return htmlAttrType;
		}
...
function extractProps(scriptContent: string, typesContent: string): PropInfo[] {
	...
	// Match destructured props from $props() - support both let and const
	const propsMatch = scriptContent.match(
		/(?:let|const)\s*\{([^}]+)\}\s*:\s*(\w+)\s*=\s*\$props\(\)/s
	);
...
function extractTypeInterface(
	...
	const interfaceRegex = new RegExp(
		`(?:export\\s+)?interface ${escapedTypeName}[^{]*\\{([^}]+)\\}`,
		's'
	);
...
function generateDocBlock(doc: ComponentDoc): string {
	const sections: string[] = ['<!--', '@component', doc.description || `# ${doc.name}`, ''];
...
/**
 * Extract the instance script (non-module script) from a Svelte component
 */
function extractInstanceScript(content: string): string | null {
	// Find all script blocks with their attributes
	...
	// Look for the instance script (one without context="module")
	const instanceScript = scriptMatches.find(([, attrs]) => !attrs.includes('context="module"'));

	// Return instance script content, or first script if no module script exists
	return (instanceScript ?? scriptMatches[0])[2];
}
...
function processComponent(filePath: string, typesContent: string): string | null {
	...
	// Extract instance script content (not module script)
	const scriptContent = extractInstanceScript(content);
...
function main() {
	...
	// Exit with error code if there were any real errors
	if (errorCount > 0) {

...
function extractTypeInterface(
	...
	const extendsMatch = typesContent.match(
		new RegExp(`(?:export\\s+)?interface ${typeName}\\s+extends\\s+([\\w,\\s]+)\\s*\\{`)
	);
	if (extendsMatch) {
